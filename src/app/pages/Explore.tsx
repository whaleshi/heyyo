...
import { ayooTokenUrl, money } from '../model';
...
export function Explore({ showMechanism }: { showMechanism: () => void }) {
  ...
  return <>
    ...
    {showPlatformToken && <section className="heyyo-strip" aria-label={l('HEYYO overview', 'HEYYO 概览')}><div className="platform-identity"><img src="/heyyo-logo.png" alt="HEYYO"/><div><h2>$HEYYO <span>{l('THE COMMUNITY TOKEN', '社区代币')}</span></h2><p>{l('Every little wave gives back.', '每一次涟漪，都有回馈。')}</p></div></div><div className="strip-stat"><span>{l('Total bought back & burned', '累计回购并销毁')}</span><strong>0<span> $HEYYO</span></strong></div><div className="strip-stat"><span>{l('Creator rewards', '创作者奖励')} </span><strong>0<span> USDC</span></strong></div><button className="strip-link" onClick={showMechanism}><span>50% {l('buyback & burn', '回购并销毁')}<br/>50% {l('to creators', '创作者奖励')}</span><ArrowUpRight size={19}/></button></section>}
