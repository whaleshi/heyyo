import { usePlatformStats, platformAmount } from '../indexer/use-platform-stats';
...
export function Explore({ showMechanism }: { showMechanism: () => void }) {
  ...
  const platform = usePlatformStats(showPlatformToken);
  ...
  return <>
    ...
    {showPlatformToken && <section className="heyyo-strip" aria-label={l('HEYYO overview', 'HEYYO 概览')}><div className="platform-identity"><img src="/heyyo-logo.png" alt="HEYYO"/><div><h2>$HEYYO <span>{l('THE COMMUNITY TOKEN', '社区代币')}</span></h2><p>{l('Every little wave gives back.', '每一次涟漪，都有回馈。')}</p></div></div><div className="strip-stat"><span>{l('Total burned', '累计销毁')}</span><strong>{platformAmount(platform?.burnedHeyyo)}<span> $HEYYO</span></strong></div><div className="strip-stat"><span>{l('Creator rewards', '创作者奖励')} </span><strong>{platformAmount(platform?.creatorRewardsUsdc)}<span> USDC</span></strong></div><a className="strip-link" href={ayooTokenUrl({contractAddress:platformTokenAddress})} target="_blank" rel="noopener noreferrer" aria-label={l('View HEYYO on Ayoo', '在 Ayoo 查看 HEYYO')}><span>50% {l('buyback & burn', '回购并销毁')}<br/>50% {l('to creators', '创作者奖励')}</span><ArrowUpRight size={19}/></a></section>}
